-- Apply only the reseller system without recreating existing types
-- This assumes app_role enum already exists with 'reseller' value
...
CREATE TABLE public.reseller_products (
  ...
  CONSTRAINT valid_reseller_price CHECK (reseller_price <= original_price * (1 + (markup_percentage / 100)))
);
...
CREATE POLICY "Resellers can manage own products" ON public.reseller_products
  FOR ALL USING (EXISTS (SELECT 1 FROM public.reseller_profiles WHERE id = reseller_products.reseller_id AND user_id = auth.uid()));
...
CREATE POLICY "Resellers can view own sales" ON public.reseller_sales
  FOR SELECT USING (EXISTS (SELECT 1 FROM public.reseller_profiles WHERE id = reseller_sales.reseller_id AND user_id = auth.uid()));
